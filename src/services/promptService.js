...
async function sendPrompt(ctx, prompt) {
  ...
  const isFavorite = Boolean(user?.favorites?.some(id => String(id) === String(prompt._id)));
  ...
  return ctx.reply(buildPromptText(prompt), {
    parse_mode: 'HTML',
    reply_markup: promptButtons(prompt, isFavorite)
  });
}
